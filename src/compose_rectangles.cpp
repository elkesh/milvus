...
int** generate_borders(cv::Mat image){
  ...
  for(i=0;i<rows;i++){
      for(j=0;j<cols;j++){
          if(image.at<cv::Vec3b>(i,j)[0]==0){                
              ...
              while(true){
                  j++;
                  if(image.at<cv::Vec3b>(i,j+1)[0]!=0){
  ...
  for(i=0;i<=rectangle_index;i++){
      ...
      std::cout<<i<<std::endl;
      std::cout<<"x1:"<<rectangles[i][0]<<std::endl;
      std::cout<<"y1:"<<rectangles[i][1]<<std::endl;
      std::cout<<"x2:"<<rectangles[i][2]<<std::endl;
      std::cout<<"y2:"<<rectangles[i][3]<<std::endl;
